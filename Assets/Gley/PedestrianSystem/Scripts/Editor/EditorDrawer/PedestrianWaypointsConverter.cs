    ...
    internal class PedestrianWaypointsConverter
    {
        ...
        private void AssignPedestrianWaypointsToCell()
        {
            ...
            for (int i = 0; i < allPedestrianEditorWaypoints.Length; i++)
            {
                ...
                {
                    var cellData = gridDataHandler.GetCell(allPedestrianEditorWaypoints[i].transform.position);
                    gridDataHandler.AddPedestrianWaypoint(cellData, i);
                    ...
                    if (allPedestrianEditorWaypoints[i].InIntersection == false)
                    {
                        ...
                        {
                            gridDataHandler.AddPedestrianSpawnWaypoint(cellData, i, allPedestrianEditorWaypoints[i].AllowedPedestrians.Cast<int>().ToArray(), allPedestrianEditorWaypoints[i].priority);
                        }
                    }
                }
            }
