...
namespace Gley.PedestrianSystem.Internal
{
    ...
    internal class PathFindingManager
    {
        ...
        private readonly PathFindingDataHandler _pedestrianPathFindingDataHandler;
        private readonly AStar _aStar;


        internal PathFindingManager(GridDataHandler gridDataHandler, PathFindingDataHandler pedestrianPathFindingDataHandler)
        {
            _gridDataHandler = gridDataHandler;
            _pedestrianPathFindingDataHandler = pedestrianPathFindingDataHandler;
            _aStar = new AStar();
        }
        ...
        internal List<int> GetPathToDestination(int pedestrianIndex, int currentWaypointIndex, Vector3 position, PedestrianTypes pedestrianType)
        {
            ...
            List<int> path = _aStar.FindPath(currentWaypointIndex, closestWaypointIndex, (int)pedestrianType, _pedestrianPathFindingDataHandler.GetPathFindingWaypoints());
        ...
        internal List<int> GetPath(Vector3 startPosition, Vector3 endPosition, PedestrianTypes pedestrianType)
        {
            ...
            var path = _aStar.FindPath(startIndex, endIndex, (int)pedestrianType, _pedestrianPathFindingDataHandler.GetPathFindingWaypoints());
        ...
        {
            List<int> possibleWaypoints = _gridDataHandler.GetPedestrianWaypointsAroundPosition(position);
            ...
            {
                if (_pedestrianPathFindingDataHandler.GetAllowedAgents(waypoint).Contains(type))
                {
                    float newDistance = Vector3.SqrMagnitude(_pedestrianPathFindingDataHandler.GetWaypointPosition(waypoint) - position);
