...
namespace Gley.PedestrianSystem.Internal
{
    ...
    internal class PedestrianDensityManager
    {
        ...
        internal PedestrianDensityManager(AllPedestrians allPedestrians, PedestrianWaypointManager waypointManager, PedestrianWaypointsDataHandler pedestrianWaypointsDataHandler, GridDataHandler gridDataHandler, Transform[] activeCameras, int maxNumberOfPedestrians, Vector3 position, Vector3 direction, float minDistanceToAdd, LayerMask trafficLayer, LayerMask buildingsLayers,
            PedestrianPositionValidator positionValidator, int activeSquaresLevel, bool useWaypointPriority, int initialDensity, bool debugDensity)
        {
            ...
            {
                gridCells.Add(_gridDataHandler.GetCell(activeCameras[i].position.x, activeCameras[i].position.z));
            }
        ...
        {
            List<SpawnWaypoint> possibleWaypoints = _gridDataHandler.GetPedestrianSpawnWaypoipointsAroundPosition(position, (int)type);
