...
namespace Gley.PedestrianSystem.Internal
{
    ...
    internal class DebugManager
    {
        ...
        internal DebugManager(PedestrianDebugSettings debugSettings, AllPedestrians allPedestrians, PedestrianWaypointManager waypointManager, PedestrianWaypointsDataHandler waypointsDataHandler, WaypointActivator waypointActivator, AllCrossings allCrossings, GridDataHandler gridDataHandler, PathFindingDataHandler pathFindingDataHandler)
        {
        ...
        private void DebugPlayModeWaypoints()
        {
            ...
            {
                Vector3 position;
                var allWaypoints = _gridDataHandler.GetAllPedestrianPlayModeWaypoints();
        ...
        private void DebugSpawnWaypoints()
        {
            if (Application.isPlaying)
            {
                var allWaypoints = _gridDataHandler.GetAllPedestrianSpawnWaypoints();
                for (int i = 0; i < allWaypoints.Count; i++)
                {
                ...
                if (pedestrians[i].CurrentBehaviour != null)
                {
                    ...
                    if (_debugSettings.DebugPathFinding)
                    {
                        if (_waypointManager.HasCustomPath(pedestrians[i].PedestrianIndex))
                        {
                            ...
                            {
                                Gizmos.color = Color.red;
                                var position = _pathFindingDataHandler.GetWaypointPosition(n);
