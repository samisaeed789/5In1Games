...
namespace Gley.PedestrianSystem.Internal
{
    ...
    internal class PedestrianManager : MonoBehaviour
    {
        ...
        public GridDataHandler GridDataHandler
        {
            get
            {
        ...
        private PathFindingDataHandler _pedestrianPathFindingDataHandler;
        public PathFindingDataHandler PedestrianPathFindingDataHandler
        {
            get
            {
                if (_pedestrianPathFindingDataHandler != null)
                {
                    return _pedestrianPathFindingDataHandler;
                }
        ...
        internal void Initialize(Transform[] activeCameras, int maxNumberOfPedestrians, PedestrianPool pedestrianPool, PedestrianSystemOptions options)
        {
            ...
            if (PedestrianModules.PathFinding)
            {
                if (MonoBehaviourUtilities.TryGetObjectScript<PathFindingData>(PedestrianSystemConstants.PlayHolder, out var resultPedestrianPathFindingWaypoints))
                {
                    ...
                    {
                        _pedestrianPathFindingDataHandler = new PathFindingDataHandler(resultPedestrianPathFindingWaypoints.Value);
                    }
                    else
                    {
                        Debug.LogError(error);
                ...
                _pathFindingManager = new PathFindingManager(GridDataHandler, PedestrianPathFindingDataHandler);
            }
            ...
            _waypointManager = new PedestrianWaypointManager(maxNumberOfPedestrians, GridDataHandler, PedestrianWaypointsDataHandler);
            WaypointManager.SetSpawnWaypointSelector(options.SpawnWaypointSelector);

            _waypointActivator = new WaypointActivator(options.DisableWaypointsArea, PedestrianWaypointsDataHandler, GridDataHandler);

            _densityManager = new PedestrianDensityManager(AllPedestrians, WaypointManager, PedestrianWaypointsDataHandler, GridDataHandler, activeCameras, maxNumberOfPedestrians, activeCameras[0].position, activeCameras[0].forward, _minDistanceToAdd, layerSetup.PedestrianLayers, layerSetup.BuildingsLayers,
            ...
            if (PedestrianModules.PathFinding)
            {
                _debugManager = new DebugManager(debugSettings, AllPedestrians, WaypointManager, PedestrianWaypointsDataHandler, WaypointActivator, AllCrossings, GridDataHandler, PedestrianPathFindingDataHandler);
            }
            else
            {
                _debugManager = new DebugManager(debugSettings, AllPedestrians, WaypointManager, PedestrianWaypointsDataHandler, WaypointActivator, AllCrossings, GridDataHandler, null);
            }
#endif
